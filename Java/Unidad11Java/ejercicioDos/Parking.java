...
public class Parking {
    ...
    public void aparcar(Coche coche) throws Exception {
        if (listaMatriculasCochesAparcados.size()<10) {
            listaMatriculasCochesAparcados.push(coche);
        } else {
            throw new Exception("Sin capacidad en su parking.");
        }
    }

    public Coche desAparcar(Coche coche) throws Exception {
        /* if (cochesAparcados.size()>0) {
            cochesAparcados.removeLast();
        } else {
            throw new Exception("Sin coches en su parking.");
        } */
        if (!listaMatriculasCochesAparcados.isEmpty() && coche.getMATRICULA()==listaMatriculasCochesAparcados.peekLast().getMATRICULA()) {
            return listaMatriculasCochesAparcados.pop();
    ...
    public void mostrarCochesAparcados() {
        ...
        //Alternativa GPT:
        listaMatriculasCochesAparcados.forEach(coche -> System.out.println(coche));
    }
